...
DISK_INFO=$(parted -l 2>&1 |grep /dev/$DISK_ID ) 
 ...
 if [ -z "$DISK_INFO"]
  then
   echo "No such disk please try again.\n"
   select_disks
 fi
...
sudo echo $unit_id > $IMG_MOUNT_PATH$UNIT_ID_PATH

##change the hostname
CURRENT_HOSTNAME=`cat $HOSTNAME_PATH | tr -d " \t\n\r"`
NEW_HOSTNAME="ERGO-$unit_id"
sudo echo $NEW_HOSTNAME > $HOSTNAME_PATH 
...
let unit_id+=1
sudo echo $unit_id > $UNIT_ID_FILE_NAME
...
sudo echo $unit_id > UNIT_ID_PATH 

##change the hostname
CURRENT_HOSTNAME=`cat $HOSTNAME_PATH | tr -d " \t\n\r"`
NEW_HOSTNAME="ERGO-$unit_id"
sudo echo $NEW_HOSTNAME >  $IMG_MOUNT_PATH/etc/hostname
sudo sed -i "s/127.0.1.1.*$CURRENT_HOSTNAME/127.0.1.1\t$NEW_HOSTNAME/g"  $IMG_MOUNT_PATH/etc/hosts
}
